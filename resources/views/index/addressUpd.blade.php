    ...
    <div class="maincont">
     ...
     <div class="head-top">
      ...
      <div class="lrBox">
        ...
        <input type="hidden" value="{{$value->address_id}}" name="address_id" id="address_id">
       <div class="lrList"><input type="text" placeholder="收货人" value="{{$value->address_name}}" name="address_name" id="address_name"/></div>
       <div class="lrList"><input type="text" placeholder="详细地址" value="{{$value->address_desc}}" name="address_desc" id="address_desc"/></div>
       <em>地址</em>
       ...
       <div class="lrList"><input type="text" placeholder="手机" value="{{$value->address_tel}}" name="address_tel" id="address_tel"/></div>
       @if($value->address_default==1)
       <div class="lrList2">设为默认<input type="checkbox" checked id="address_default"></div>
       @else
      ...
      <div class="lrSub">
       <input type="javascript:;" id="tianjia" value="修改" />
      </div>
...
<script>
    ...
    $(document).on('click','#tianjia',function(){
            ...
            //修改
            $.get(
              "{{url('index/addressUpddo')}}",
              obj,
              function(res){
                //console.log(res);
                  if(res==1){
                    layer.msg('修改成功',{icon:res},function(){  
                      location.href="{{url('index/address')}}";                 
                    });
                  }else{
                    layer.msg('修改失败',{icon:res},function(){                     
                    });
                  }
              }
              ,'json'
